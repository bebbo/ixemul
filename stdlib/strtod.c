...
#endif
{
 /*	Arguments ndigits, decpt, sign are similar to those
	...
	*/
	...
	if (u.u_result) {
		((Bigint *)u.u_result)->k = u.u_result_k;
		((Bigint *)u.u_result)->maxwds = 1 << u.u_result_k;
		Bfree(u.u_result);
		u.u_result = 0;
		}
	if (word0(d) & Sign_bit) {
	...
	j = sizeof(ULong);
	for(u.u_result_k = 0; sizeof(Bigint) - sizeof(ULong) + j <= i;
		j <<= 1) u.u_result_k++;
	u.u_result = Balloc(u.u_result_k);
	s = s0 = (char *)u.u_result;
